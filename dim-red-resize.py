...
avg_img = torch.zeros((RESIZE_DIM**2*3,),device=DEVICE)
vec_i_batch = []
for i, f_i in tqdm(enumerate(image_files)):
    vec_i = preprocess_image_resize(f_i, RESIZE_DIM, DEVICE)
    avg_img += vec_i
    ...
    if len(vec_i_batch) == CALC_BATCH_SIZE:
        ...
        if vec_i is not None:
            ...
            for j, f_j in tqdm(enumerate(image_files[:i+1])):
                vec_j = preprocess_image_resize(f_j, RESIZE_DIM, DEVICE)
...
torch.save(HTH,f"saved_matrices/HTH_{RESIZE_DIM}.pt")
torch.save(avg_img,f"saved_matrices/mu_{RESIZE_DIM}.pt")
torch.save(sigma,f"saved_matrices/sigma_{RESIZE_DIM}.pt")
